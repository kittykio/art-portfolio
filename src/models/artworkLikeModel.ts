 ...
 */
export interface IArtworkLikeDocument extends IArtworkLike {
  createdAt: Date;
  updatedAt: Date;
}
 ...
 */
export const ArtworkLikeSchema = new Schema<IArtworkLikeDocument>(
  {
    _id: {
      type: Number,
      required: true,
...
let ArtworkLikeModel: Model<IArtworkLikeDocument>;
...
if (mongoose.models.Artwork) {
  ...
  ArtworkLikeModel = mongoose.models.Artwork as Model<IArtworkLikeDocument>;
} else {
  // Create the new model.
  ArtworkLikeModel = mongoose.model<IArtworkLikeDocument>('Artwork', ArtworkLikeSchema);
}

// Export the model for use.
export default ArtworkLikeModel;
