...
export const em: FC<ComponentPropsWithoutRef<'em'>> = (props) => (
  <em {...props} className="italic tracking-wider text-dglg-900" />
);
...
//   <code
//     {...props}
//     className="bg-dglg-900 text-dglg-200 px-1.5 py-0.5 rounded text-sm font-mono"
//   />
// );
...
export const pre: FC<ComponentPropsWithoutRef<'pre'>> = (props) => (
  <pre {...props} className="bg-dglg-950 p-4 rounded-lg overflow-x-auto my-4 text-sm font-mono" />
);
...
export const table: FC<ComponentPropsWithoutRef<'table'>> = (props) => (
  <table {...props} className="w-full border-collapse border border-dglg-700 my-4 text-sm" />
);
...
export const thead: FC<ComponentPropsWithoutRef<'thead'>> = (props) => (
  <thead {...props} className="bg-dglg-800" />
);
...
export const tr: FC<ComponentPropsWithoutRef<'tr'>> = (props) => (
  <tr {...props} className="border-b border-dglg-700" />
);
...
export const input: FC<ComponentPropsWithoutRef<'input'>> = (props) =>
  props.type === 'checkbox' ? (
    <input {...props} className="mr-2 accent-dglg-600" />
  ) : (
    <input {...props} />
  );
 ...
 */
export const Divider: FC = () => <hr className="flex-grow border-t-2 border-dglg-700 my-6" />;
...
export const Spacey: FC<FontProps> = ({ block, children, ...props }) => {
  ...
  return (
    <Tag
      {...props}
      className={`font-spacey bg-dglg-900 text-lgdg-100 px-2 tracking-widest ${
        block ? 'my-2' : ''
      }`}
    >
      {children}
...
export const Highlight: FC<HighlightProps> = ({
  ...
  const colorClasses = colorClassMap[color] || `bg-${color} text-dglg-700`;

  return (
    <Tag
      {...props}
      className={`${colorClasses} rounded-md 
        ${block ? 'my-2 px-2 py-1 inline-block' : 'inline p-1'}
      `}
    >
      {children}
    </Tag>
...
export const GradientHighlight: FC<HighlightProps> = ({
  color = 'lemon',
  block = false,
  children,
  ...props
  ...
  const colorClassMap: Record<string, string> = {
    flame: 'from-flame-300 via-flame-500 to-flame-900 text-gray-100',
    lemon: 'from-lemon via-yellow-300 to-flame-300 text-gray-900',
  };

  const colorClasses = colorClassMap[color] || `from-${color}-500 to-${color}-700 text-dglg-700`;

  return (
    <Tag
      {...props}
      className={`bg-gradient-to-r ${colorClasses} rounded-md
        ${block ? 'my-2 px-2 py-1 inline-block' : 'inline p-1'}
      `}
    >
      {children}
